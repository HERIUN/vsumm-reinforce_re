...
parser.add_argument('--input', type=str, required=True, help="input summe video path ex. ./SuMe/")
...
class Generate_Dataset:
    ...
    def _set_video_list(self, video_path):
        if os.path.isdir(video_path):
            ...
            self.video_list = [file for file in self.files_list if file.endswith(".mp4")]
    ...
    def generate(self):
        for idx, file_name in enumerate(self.video_list):
            ...
            self.h5_file['video_{}'.format(idx+1)]['features'] = np.array(feature_stack_for_train)
            self.h5_file['video_{}'.format(idx+1)]['picks'] = np.array(list(picks))
            self.h5_file['video_{}'.format(idx+1)]['n_frames'] = n_frames
            self.h5_file['video_{}'.format(idx+1)]['change_points'] = self.origin_h5[f'video_{idx+1}']['change_points'][...]
            self.h5_file['video_{}'.format(idx+1)]['gtscore'] = self.origin_h5[f'video_{idx+1}']['gtscore'][...]
            self.h5_file['video_{}'.format(idx+1)]['gtsummary'] = self.origin_h5[f'video_{idx+1}']['gtsummary'][...]
            self.h5_file['video_{}'.format(idx+1)]['n_frame_per_seg'] = self.origin_h5[f'video_{idx+1}']['n_frame_per_seg'][...]
            self.h5_file['video_{}'.format(idx+1)]['n_steps'] = self.origin_h5[f'video_{idx+1}']['n_steps'][...]
            self.h5_file['video_{}'.format(idx+1)]['user_summary'] = self.origin_h5[f'video_{idx+1}']['user_summary'][...]
